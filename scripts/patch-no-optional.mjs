...
// Remove existing new tool blocks cleanly
idx = idx.replace(/\/\/ ─── TOOL: get_page[\s\S]*?(?=\/\/ ─── (?:TOOL:|START))/g, '');
idx = idx.replace(/\/\/ ─── TOOL: update_page_content[\s\S]*?(?=\/\/ ─── (?:TOOL:|START))/g, '');
idx = idx.replace(/\/\/ ─── TOOL: web_search[\s\S]*?(?=\/\/ ─── (?:TOOL:|START))/g, '');
...
['get_page', 'update_page_content', 'web_search'].forEach(t => {
  console.log(verify.includes(`"${t}"`) ? `✓ ${t}` : `✗ ${t}`);
});
console.log('Total tools:', (verify.match(/server\.tool\(/g) || []).length);
